...
export const forBanner = (item, unit) => {
  if (!unit.standardBearer) {
    return item.itemType !== ITEM_TYPE_BANNER;
  }
  return true;
};
...
export const forMusicians = (item, unit) => {
  if (!unit.musician) {
    return item.itemType !== ITEM_TYPE_MUSICIAN;
  }
  return true;
};
...
export const forCrossBows = (item, unit) => {
  if (unit.rangedWeapon === NO_RANGE_WEAPON || !CROSSBOW_TYPES.includes(unit.rangedWeapon)) {
    return item.itemType !== ITEM_TYPE_BOWS;
  }
  return true;
};
...
export const forBows = (item, unit) => {
  if (unit.rangedWeapon === NO_RANGE_WEAPON || !BOW_TYPES.includes(unit.rangedWeapon)) {
    return item.itemType !== ITEM_TYPE_CROSSBOWS;
  }
  return true;
};
...
export const whenUnitIsGiant = (item, unit) => {
  if (unit.unitType === GIANT && BOW_TYPES.includes(unit.rangedWeapon) ) {
    return item.itemType === ITEM_TYPE_CROSSBOWS || item.itemType === ITEM_TYPE_BOWS;
  }
  return true;
};
