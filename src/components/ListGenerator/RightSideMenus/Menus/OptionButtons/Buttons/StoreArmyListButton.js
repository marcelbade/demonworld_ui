...
const StoreArmyListButton = (props) => {
  ...
  return (
    <>
      <Tooltip title={<Typography sx={{ fontSize: "20px" }}>{OPTIONS.STORE_LIST}</Typography>}>
        <span>
          <IconButton
            disabled={SEC.selectedUnits.length === 0} //
            onClick={() => {
              props.setIsExistingList(false);
...
export default StoreArmyListButton;
